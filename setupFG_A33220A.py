...
#        print "Connecting to Agilent Function Generator at "+IP
 ...
 #       self.fg.settimeout(1.0) # make future timeouts much faster  
    ...
    def set(self):
        waveforms=["SIN","SQU","RAMP","PULS","NOISE","DC"]
        if self.shape in waveforms:
            bck=self.fg.send("FUNC "+self.shape+"\n")
        else: 
            print("ERROR unknown wave form: should be one of :")
            print(waveforms)
            print("you gave "+self.shape)
            raise Exception("Unknown wave form error")
        ...
        if self.amp < 0.01:
            self.amp = 0.01
        if ((self.offset+self.amp) > 0.55) or ((self.offset-self.amp) < -0.55):
            raise Exception("Voltage waveform must not exceed +-0.55V")
        ...
        if chk_amp != self.amp :
            ...
            raise Exception("Failure to set amplitude")
...
if __name__ == "__main__":
    ...
    if (len(sys.argv) < 3):
        ...
        fg=setupFG.util(freq,amp,offset=offset,shape=shape,onoff=onoff)
        fg.set()
