...
#      - added definition for toggleHEMT(), but isn't working yet
...
#   Dec2014 - sms
#	- fixed the bug that was causing getRawPhaseLong to hang
class util:
...
#        self.frmAmp=0.139          # for 1077ohm bias resistors
        ...
        self.fluxRamp=[1000.,0.4,0,'ramp','on'] # default FR freq,amp,offset,wave,state
    ...
    def autoAdcRange(self,ADC_max=1950,ADC_min=1000):
        ...
        while True :
            ...
            i,q = self.getSnapAdc()
            #print 'got here'
            maxval = max(max(i)[0],max(q)[0],abs(min(i)[0]),abs(min(q)[0]))
            n_itter += 1
            if maxval > ADC_max :
                #print "over value"
                self.atten_in += d_attens
                if self.atten_in > 31.5:
                    print('WARNING - ADC counts are still saturated with maximum input attenuation.')  
                    self.atten_in = 31.5
                    break
...
#        for ii in range(self.Nfreqs): <- findRes() doesn't change Nfreqs
    ...
    def setupFG(self,freq,amp,off,wave,out):
        ...
        'output=[on,off]'
        os.system('/opt/local/bin/python2.7 /export/home/egret/monctrl/umux/setupFG.py %s %s %s %s %s'%(freq,amp,off,wave,out));
	...
	maxv=0.4
        ...
        rootdir=argv[1]
        freqFile=rootdir+'default_freqs.txt'
        ...
        self.sweepLO(self.freqs,self.loFreq,loSpanShort,atten_in,atten_out)

        pl.figure(f_num);  f_num += 1
        # make a mag(s21) vs freq plot:
        print "plotting s21 vs freq..."
        for ii in range(self.Nfreqs):
            semilogy(array(self.f_span[ii])/1.e+9, (self.I[ii,:]**2 + self.Q[ii,:]**2)**.5,'k.-');
        pl.axis('tight');
        pl.ax=gca(); xmin,xmax=ax.get_ylim();
        ...
        for ii in range(self.Nfreqs):
            ...
            pl.plot(self.iq_centers[ii].real,self.iq_centers[ii].imag,'ro');
            pl.xlabel('I (adu)'); pl.ylabel('Q (adu)'); pl.title('raw IQ loops'); pl.axis('equal'); pl.draw(); pl.draw();
        if plot2screen: 
            pl.show(block=False); 
        if saveplot: 
        ...
        # Step in DC voltage up to max(voff)
        # For each voff value, sweep the LO through loFreq +/- loSpanShort/2
        ...
        for ii in range(Nch):
            for jj in range(Natt):
                ...
                # show response
                pl.subplot(nrow,ncol,ii+1).plot((xx[0]-mean(xx[0]))/1.e+6,xx[1],'-'); 
                # show fit
                pl.subplot(nrow,ncol,ii+1).plot((xxx[0]-mean(xx[0]))/1.e+6,fitfunc(ppOut,xxx),'r-');
                # show selected tone
                pl.subplot(nrow,ncol,ii+1).plot((freqFits[ii,jj]-mean(xx[0]))/1.e+6,xx[1][argmax(xx[1])],'gx');
                pl.ax=gca();
                ...
                if (ii != 32):
                    pl.ax.yaxis.set_ticklabels(''); pl.ax.xaxis.set_ticklabels('');
                else:
                    pl.ax.xaxis.label.set_fontsize(fs); pl.ax.yaxis.label.set_fontsize(fs)
                for tick in pl.ax.xaxis.get_major_ticks():
                    pl.tick.label1.set_fontsize(fs)
                for tick in pl.ax.yaxis.get_major_ticks():
                    pl.tick.label1.set_fontsize(fs)
        ...
        self.atten_out=array([self.attens.min()])
        self.progAttens(self.atten_in,self.atten_out)
        self.defineLUTs()
    ...
    def setupDemod(self,frmFreq,frmAmp,carFreqs='measure',clip=True,doFG=True):
        ...
        if(carFreqs == 'measure'):
            ...
            if clip == True :
                mcf=median(carFreqs[carFreqs>0])
        ...
        meanCarFreq=mean(carFreqs[carFreqs>0])
    ...
    def vna(self,fstart,fstop, polyorder=2, phase='no',df=1e4):
        ...
        for ii in range(self.Nfreqs):
            ...
            if phase=='yes':
                phasedat=unwrap(arctan2(self.Q[ii,:],self.I[ii,:]),axis=0)
    ...
    def setNreadoutCh(self,NchRead):
        ...
        numFrames=int(floor(bram_size/packetBytes))
...
#        self.roach.write_int('channel_select',NchRead)
    ...
    def calcLO(self,tod,sampPerFrame,frmFreq):
        ...
        ac=correlate(toda,toda,mode="full")
        mv=ac[0]
        for ii in arange(len(ac)-1):
            if mv < ac[ii]:
                max_ind = ii
                mv = ac[ii]
        first_min=max_ind
        for ii in (arange(len(ac)-max_ind-1)+max_ind):
            if mv > ac[ii]:
                first_min = ii
                mv = ac[ii]
        second_max=first_min
        for ii in (arange(len(ac)-first_min-1)+first_min):
            if mv < ac[ii]:
                second_max = ii
                mv = ac[ii]
        second_min=second_max
        for ii in (arange(len(ac)-second_max-1)+second_max):
            if mv > ac[ii]:
                second_min = ii
                mv = ac[ii]
        dt=[first_min-max_ind,second_max-first_min,second_min-second_max]
        carrierFreq=0.5/(mean(dt))*self.chSampleRate
        LOsin = (max(toda)-min(toda))/2.0*sin(2.0*pi*arange(len(toda))*carrierFreq/self.chSampleRate)+ mean(toda)
        LOcos = (max(toda)-min(toda))/2.0*cos(2.0*pi*arange(len(toda))*carrierFreq/self.chSampleRate)+ mean(toda)
    ...
    def findRes(self,freqs,mags, width=6e6,minsep=3e6, resband=3e5, hwhmcut=3, ampcut=0.5, polyorder=5, mfwid=3e6):
        ...
        inds=[] 
        for i in linspace(1,len(mags)-2,len(mags)-2):
            if i < ws/2:
                max_sig=-std(yvals[0:i+ws/2])
            elif i > (len(mags)-ws/2):
                max_sig=-std(yvals[i-ws/2:len(mags)-1])
            else:
                max_sig=-std(yvals[i-ws/2:i+ws/2])
            if yvals[i] > max_sig: 
                continue
        ...
        hwhmarr=zeros(len(inds))
        amparr=zeros(len(inds))
        ...
        wgood = ( (hwhmarr > 0.3*median(hwhmarr)) & (hwhmarr < 3.0*median(hwhmarr)) &
                  strongneg)
    ...
    def mad(self,x,axis=None):
        ...
        z=x-xm
        gi=where(z != 0.0)
        return median(absolute(z[gi]-xm),axis)
    ...
    def stitchSweep(self,fstart,fstop,saveraw=0):
        ...
        mg1=mg1[0:len(fr1)]
        freqs=concatenate([fr1,fr2])
        mags=concatenate([mg1,mg2])
        print "Stitched and found %s resonances"%str(len(freqs))
        if saveraw!=0:
            fdat1=fdat1[fdat1<min(fdat2)]
            mag1=mag1[0:len(fdat1)]
            fraw=concatenate([fdat1,fdat2])
            mraw=concatenate([mag1,mag2])
